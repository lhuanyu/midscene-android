...
public class SelfCheckScriptTest {
    ...
    private static final String CHANNEL =
            "/storage/emulated/0/Android/data/com.midscene.android/files/channel";
    ...
    @Test
    public void aLocalizedConsoleIsDescribedWithItsOwnCopy() {
        String script = SelfCheckScript.config(false, CHANNEL, new SelfCheckScript.Anchors(
                "运行", "脚本", "历史", "指令", "例如：打开设置并搜索 Wi-Fi"));

        // Every anchor is what the console renders right now, so a Chinese console is
        // not described with the English tab names, label or placeholder.
        assertTrue(script.contains("「运行」页里的 指令 标题下方的指令输入框"));
        assertTrue(script.contains("例如：打开设置并搜索 Wi-Fi"));
        assertTrue(script.contains("底部标签栏里的 脚本 标签"));
        assertTrue(script.contains("底部标签栏里的 历史 标签"));
        assertFalse(script.contains(SelfCheckScript.INSTRUCTION_LABEL + " 标题下方"));
        assertFalse("an English tab name is not on a Chinese console",
                script.contains("Scripts 标签"));
    }
}
