...
with `pnpm --dir apps/android-host runtime -- --from <runtime-dir>`.
...
```bash
pnpm run build            # build the agent runtime package (from the repository root)
pnpm test                 # its unit tests
pnpm run test:android     # these JVM tests
```

The first two commands run in the regular workspace. The Android JVM tests
...
meant to be handed to someone, `pnpm --dir apps/android-host dist:release` copies
